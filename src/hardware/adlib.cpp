...
namespace OPL2BOARD {
	...
	struct Handler : public Adlib::Handler {
		Handler(const char* port, long unsigned int* oplbaud) {
			opl2AudioBoard.connect(port, oplbaud);
		}
...
namespace OPL3DUOBOARD {
	...
	struct Handler : public Adlib::Handler {
		Handler(const char* port, long unsigned int* oplbaud) {
			opl3DuoBoard.connect(port, oplbaud);
		}
		virtual void WriteReg(uint32_t reg, uint8_t val) {
...
Module::Module( Section* configuration ) : Module_base(configuration) {
	...
	unsigned long int opl_baud = (unsigned long int)section->Get_int("oplbaudrate");
